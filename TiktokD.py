...
banner = f"""{war}
  TIKTOK DOWNLOAD
   {M}• {P}Author {M}: {P}AvinYetee
   {M}• {P}Github {M}: {K}/NakanoYetee
   {M}• {P}Team   {M}: {H}Gak ada
 {M}• {P}Masukan Link Video Tiktok {M}({H}https://vt.tiktok.com/avinih{M})"""
...
class Main:
	...
	def dow_tt(self, tk_vid, id_vid, nama_vid):
		...
		try:
			print (f' {M}• {P}Menunggu Itu Membosankan {M}....')
	...
	def get_dat(self, url):
		...
		try:
			...
			if 'true' in data:
				...
				gas = input(f'\n {M}• {P}LanjutDownload Video {M}[{H}Y/T{M}]:{H} ')
				if gas in ['Y','y']:
					nama_vid = input(f'\n {M}• {P}Masukan Nama Video You {M}[{P}OUTPUT{M}]{M}:{H} ')
					self.dow_tt(tk_vid, id_vid, nama_vid)
				else:
					exit(f' {M}• {P}Thanks udah pake >_<')
	...
	def mulai(self):
		...
		if url in ['']:
			exit(f' {M}• {P}Masukan link Dengan Benar Coxr -_-')
		else:
			self.get_dat(url)
Main().mulai()